...
export function buildMasterSubAgentDelegateChatTools(
  ...
  const capabilityTable = [
    ...
    "",
    "🏠 life 生活全能助手",
    "   能力: wallet · purchase · entertainment",
    "   工具: wallet.transfer / wallet.recharge / wallet.purchase(全场景消费) + desktop.visual.*(电脑操控) + 游戏(五子棋/斗地主等)",
    "   场景: 涉及钱包写操作(转账/消费/充值) + 电脑操控 + 游戏对局时才委派",
        ...
        parameters: {
          ...
          properties: {
            agentType: {
              ...
              description: "Sub-agent type. Routes: life=复杂生活操作(钱包写+视觉操控+游戏), tech=技术操控(RPA+代码+运维), info=信息检索(深度调研), creative=创意内容(文案/策划/写作/PPT), security=安全审计(风险检测/审批/拦截).",
            },
            taskDescription: {
              type: "string",
